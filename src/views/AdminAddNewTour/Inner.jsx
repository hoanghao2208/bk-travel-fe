...
const Inner = memo(
    ({ handleCreateNewTour, form, fileListMap, setFileListMap }) => {
        useEffect(() => {
        ...
        const processedFiles = useMemo(() => new Map(), []);
        ...
        const handleUploadTourImageList = ({ fileList: newFileList }) => {
            ...
            const filesToSet =
                Array.from(uniqueFiles).filter(handleBeforeUpload);
        ...
        }, [fileListMap, tourImageList, processedFiles]);

        const handleRemoveImageFromList = useCallback(
            delete_url => {
                fileListMap.delete(delete_url);
                setFileListMap(new Map(fileListMap));
                const allFiles = Array.from(fileListMap.values());
                setTourImageList(allFiles);
            },
            [fileListMap, setFileListMap, setTourImageList]
        );
                                        ...
                                        <div className="add-new-tour__list-image">
                                            <Image.PreviewGroup>
                                                {Array.from(
                                                    ...
                                                    <div
                                                        key={url}
                                                        className="add-new-tour__remove-img"
                                                    >
                                                        <Image
                                                            width={200}
                                                            src={url}
